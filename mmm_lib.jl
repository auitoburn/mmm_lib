...
function find_ids!(clusters::Vector{Cluster}, glob::Params)
	seqscorelists = [Vector{Tuple{Float64,Int64}}() for _ in 1:length(clusters)]
	for n in 1:glob.nrows
	    id=glob.rowcluster[n]
	    spliceCluster!(clusters[id],n,glob)
	    push!(seqscorelists[id],(rowLikelihood(n,clusters[id],glob),n))
	    pushCluster!(clusters[id],n,glob)
	end
	for idx in 1:length(seqscorelists)
            seqscorelists[idx]=sort(seqscorelists[idx],by=x->x[1])
	end
	cluster_variances=[var(x[1] for x in seqscorelists[id]) for id in 1:length(seqscorelists)]
	indices=sortperm(cluster_variances,rev=true)
	return indices
end
...
function clean!(clusters::Vector{Cluster},glob::Params)
        idlist=Tuple{Int64,Int64}[]
        clusters=filter!(c -> c.nrows > 0, clusters)
        for i in 1:length(clusters)
            if (clusters[i].clustid!=i)
               push!(idlist,(clusters[i].clustid,i))
               clusters[i].clustid=i
            end
        end
	
        oldids=[t[1] for t in idlist]
        for i in 1:glob.nrows
                if glob.rowcluster[i] in oldids
                   id=glob.rowcluster[i]
                   glob.rowcluster[i]=idlist[findfirst(t->t[1]==id,idlist)][2]
                end
        end
...
function iterate_EM_BIC!(cl0::Cluster,glob::Params)
    ...
    check=0
    L=0
    while ((glob.nclust==0&&nold!=N)|| (glob.nclust > 0 && length(clusters) < glob.nclust))
	if glob.debug
...
function fixed(cl0::Cluster,glob::Params,idx::Int)
    ...
    while (((glob.nclust == 0)&&(ll_hm>ll_hm_last)) || (glob.nclust > 0 && length(clusters) < glob.nclust))
       ...
       ll_hm_last = ll_hm
       llr_last = llr
       clusters,ll_hm,nnew=routine(clusters,glob,idx)
       if(ll_hm<ll_hm_last)
	 if(length(id_best)>length(clusters))
	    return clusters,ll_hm
         else
	    restore_clusters!(glob,clusters,id_best)
	    return clusters,ll_hm_last
         end
       else
         id_best=save_clusters(glob,clusters)
       end
	...
	if (check==0||Check==1)
	   ...
	   if (ll_hm2>ll_hm1)
	     ...
	     if (ll_hm_last>ll_hm2)
		      println("1");glob=glob2;
		 id_best=save_clusters(glob,Clusters2)
		 nold=length(id_best)
                 l1=length(Clusters2);l2=length(clusters2);l=floor(Int,(l1+l2)/2)
		 clusters,ll_hm,nnew,max=routine(Clusters2,glob,l-l1,max)
	     else
		      println("2")
		 id_best=save_clusters(glob,clusters2)
		 nold=length(id_best)
	         l1=length(clusters2);l=floor(Int,(l1+L)/2)
		 clusters,ll_hm,nnew,max=routine(clusters2,glob,l-l1,max)
		 ll_hm_last=ll_hm2
	      end
		 ...
		 ll_hm_last=ll_hm_prev
             else
	         println("4");glob=glob1
		 id_best=save_clusters(glob,clusters1)
		 nold=length(id_best)
		 l1=length(clusters1);l2=length(Clusters2);l=floor(Int,(l1+l2)/2)
                 clusters,ll_hm,nnew,max=routine(clusters1,glob,l-l1,max)
		 ll_hm_last=ll_hm1
              end
           else
	...
	if ((ll_hm <= ll_hm_last) && (glob.nclust==0) && (go==1))
	   ...
	   if (length(id_best)<=L)
	      ...
	      return clusters
           ...
           if glob.debug
              println("(AR)iterate_EM_HM #clust=", length(clusters), " llr=",llr_last," ll_hm=",ll_hm_last)
           end
           N=div(nold+L,2)
	   m=N-2*nold
           check=1;Go=1;CHECK=1;first=0
	   ll_hm,ll_hm_last=ll_hm_last,ll_hm
        elseif(go==1)
	   if(check==1)
	      if (Check==0)	   
	         id_prev=save_clusters(glob,id_best)
              end
	      id_best=save_clusters(glob,clusters)
              N=div(nnew+L,2)
	      m=N-2*nnew
